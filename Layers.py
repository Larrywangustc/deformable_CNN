...
class ConvOffset2D(keras.layers.Layer):
    """ConvOffset2D"""
    ...
    def __init__(self, filters, init_normal_stddev=0.01, **kwargs):
        ...
        super(ConvOffset2D, self).__init__(
            trainable=True,
            **kwargs)
        self.conv2d = Conv2D(2 * self.filters, (5, 5), padding='same', use_bias=False,
                             # TODO gradients are near zero if init is zeros
                             kernel_initializer='zeros', **kwargs)
        '''        super(ConvOffset2D, self).__init__(
            50, (5, 5), padding='same', use_bias=False,
            # TODO gradients are near zero if init is zeros
    ...
    def call(self, x):
        ...
        #offsets = tf.tile(offsets, [1, 1, 1, 2])
